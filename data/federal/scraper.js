  ...
  return fetch(url)
    ...
    .then(text => {
      ...
      for (const result of list) {
        output.parties[result.party].threethirtyeight = result.seats;
      }
      return output;
    });
}
...
function getCalculatedPolitics() {
  ...
  return fetch(url)
    ...
    .then(text => {
      ...
      for (const result of list) {
        output.parties[result.party].calculatedPolitics = result.seats;
      }
      return output;
    });
}
...
async function getCBC() {
  ...
  for (const result of list) {
    output.parties[result.party].cbc = result.seats;
  }
  return output;
}
...
function getCdnElectionWatch() {
  ...
  return fetch(url)
    ...
    .then(text => {
      ...
      for (const result of list) {
        output.parties[result.party].cdnElectionWatch = result.seats;
      }
      return output;
    });
}
...
function getAverages() {
  for (const party in output.parties) {
    output.parties[party].sum = 0;
    for (const projection in output.parties[party]) {
      if (projection !== "sum") {
        output.parties[party].sum += output.parties[party][projection];
      }
    }
    output.parties[party].avg =
      output.parties[party].sum /
      (Object.keys(output.parties[party]).length - 1);

    output.parties[party].avg =
      Math.round(output.parties[party].avg * 100) / 100;

    delete output.parties[party].sum;
  }
}
